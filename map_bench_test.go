...
func BenchmarkWithSyncMap(b *testing.B) {
	syncMap := &sync.Map{}
	genericSyncMap := &Map[int, int]{}
	const hits, misses = 1, 1023
	b.Run("sync map load", func(b *testing.B) {
		for j := 0; j < b.N; j++ {
			for i := 0; i < hits; i++ {
				syncMap.LoadOrStore(i, i)
			}
			// Prime the map to get it into a steady state.
			for i := 0; i < hits*2; i++ {
				syncMap.Load(i % hits)
	...
	b.Run("generic sync map load", func(b *testing.B) {
		for j := 0; j < b.N; j++ {
			for i := 0; i < hits; i++ {
				genericSyncMap.LoadOrStore(i, i)
			}
			// Prime the map to get it into a steady state.
			for i := 0; i < hits*2; i++ {
				genericSyncMap.Load(i % hits)
			}
		}
	})
}
